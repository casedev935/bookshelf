...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const moviesWatching = movies.filter(m => m.status === 'assistindo').length;
    ...
    <div className="flex flex-col h-full bg-[var(--color-background-primary)]">
      <div className="flex-1 p-6 md:pt-4 md:px-12 md:pb-12 max-w-7xl mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <StatCard title="Total Movies" value={movies.length} colorClass="bg-[var(--color-accent)] text-white" />
          <StatCard title="Movies Watching" value={stats.moviesWatching} colorClass="bg-[var(--color-status-ongoing)] text-white" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <StatCard title="Total Books" value={books.length} colorClass="bg-[var(--color-accent)] text-white" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <StatCard title="Total Series" value={series.length} colorClass="bg-[var(--color-accent)] text-white" />
