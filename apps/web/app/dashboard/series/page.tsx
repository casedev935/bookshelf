...
export default function SeriesPage() {
  ...
  const fetchMedia = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/series`, { headers });
  ...
  const fetchCategories = async () => {
    ...
    const headers = { 'Authorization': `Bearer ${token}` };
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/categories?type=series`, { headers });
  ...
  const handleSave = async (data: any) => {
    const token = localStorage.getItem('token');
    const endpoint = `${process.env.NEXT_PUBLIC_API_URL}/series${editingMedia ? `/${editingMedia.id}` : ''}`;
    const method = editingMedia ? 'PATCH' : 'POST';
    
    // For now, let's just send the basic series data. 
    // Seasons creation/syncing can be a follow-up.
  ...
  const handleDelete = async () => {
    ...
    const token = localStorage.getItem('token');
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/series/${itemToDelete}`, {
      method: 'DELETE',
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[var(--color-background-primary)] overflow-y-auto">
      <Header title="TV Series Collection" />
        ...
        <div className="neo-brutalist bg-white p-2 mb-6 flex flex-wrap md:flex-nowrap items-end gap-2">
          ...
          <div className="flex gap-2 w-full md:w-auto mt-2 md:mt-0">
            ...
            <button
              ...
              className="neo-brutalist-button p-2 px-6 font-black bg-[#ff00ff] text-white flex-1 md:flex-none flex items-center justify-center text-xl"
              title="Add Series"
            >
              +
            </button>
        ...
        <div className={layoutMode === 'grid'
          ...
          {filteredSeries.length === 0 && (
            <div className="col-span-full text-center p-12 bg-white neo-brutalist text-gray-400 font-bold font-mono">
              NO TV SERIES FOUND MATCHING CRITERIA.
            </div>
          )}
        </div>
      </div>
      ...
      {itemToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="neo-brutalist bg-white w-full max-w-sm z-10 p-6 flex flex-col gap-4">
            ...
            <p className="font-mono text-sm font-bold">Are you sure you want to delete this series? This action cannot be undone.</p>
